...
import { FileArchive, Layers3, Loader2, Plus, Route, Upload } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {tab === "mine" ? (
          <Button
            ...
            className="h-12 rounded-xl bg-cyan-400 px-5 font-extrabold text-slate-950 hover:bg-cyan-300"
          >
            {creatingBlank ? <Loader2 className="h-5 w-5 animate-spin" /> : <Plus className="h-5 w-5" />}
            {CREATE_BLANK_FUNNEL_LABEL}
          </Button>
        ) : null}
      </div>
      ...
      {tab === "templates" ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="border-white/8 bg-card/70 p-5 lg:col-span-2">
            <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-cyan-300">Optional presets</p>
            <p className="mt-2 text-sm font-medium text-muted-foreground">
              Templates are saved starting points. Create a blank funnel from My Funnels if you want an empty canvas.
            </p>
          </Card>
          {templatesError ? (
            <Card className="border-red-400/20 bg-red-400/[0.05] p-5 font-bold text-red-200">
              {templatesError}
              <p className="mt-2 text-sm font-medium text-muted-foreground">
                Optional presets may be unavailable. Blank funnels can still be created from My Funnels.
              </p>
            </Card>
          ...
          {templates.length ? (
            templates.map(template => (
              <Card key={template.templateKey} className="border-white/8 bg-card/70 p-5">
                <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-cyan-300">
                  {template.source === "zip" ? "Imported preset" : "Optional preset"}
                </p>
                ...
                <Button
                  ...
                  className="mt-5 h-12 w-full rounded-xl bg-white/10 font-extrabold text-foreground hover:bg-white/16"
                >
            ...
            <Card className="border-white/8 bg-card/70 p-5 font-bold text-muted-foreground">
              No saved presets yet. Create a blank funnel instead.
            </Card>
          )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    {paidFunnelLibrarySourceLabel(funnel)} · {funnel.status}
                  </p>
                </div>
              </div>
              <Button
        ...
        <Card className="border-white/8 bg-card/70 p-5">
          <h3 className="text-xl font-extrabold">No funnels yet</h3>
          <p className="mt-2 text-sm font-medium text-muted-foreground">
            Start with an empty page. Templates are optional presets, not required.
          </p>
          <Button
            type="button"
            disabled={busy}
            onClick={onCreateBlank}
            className="mt-5 h-12 rounded-xl bg-cyan-400 px-5 font-extrabold text-slate-950 hover:bg-cyan-300"
          >
            {creatingBlank ? <Loader2 className="h-5 w-5 animate-spin" /> : <Plus className="h-5 w-5" />}
            {CREATE_BLANK_FUNNEL_LABEL}
          </Button>
