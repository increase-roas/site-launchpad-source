...
function generatedFiles() {
  return compilePaidFunnelToAstro(createGenericPaidFunnelFixture("offline-gate"));
}
...
describe("offline conversion contract v1 release gate", () => {
  it("keeps Simple Form and generic Astro funnels on the exact same canonical contract", () => {
    ...
    expect(SIMPLE_FORM_OFFLINE_CONVERSION_CONTRACT.stageMappings).toEqual([
      { pipelineStage: "Hot Pursuit", callbackStage: "qualified", metaEvent: "QualifiedLead" },
      { pipelineStage: "Appointment Set", callbackStage: "appointment", metaEvent: "Schedule" },
  ...
  it("uses one browser event ID for Pixel and server CAPI and retains original attribution", () => {
    ...
    const migration = files.find(file => file.path === "migrations/0001_funnel_events.sql")?.contents ?? "";
    ...
    expect(runtime).toContain('const ATTRIBUTION_KEYS = ["utm_source","utm_medium","utm_campaign","utm_content","utm_term","fbclid","gclid"]');
    ...
    for (const field of SIMPLE_FORM_OFFLINE_CONVERSION_CONTRACT.originalAttribution.fields) {
      expect(migration).toContain(field);
    }
  });

  it("upserts GHL, deduplicates Sheet appends, and uses only the service-account pair", () => {
    const endpoint = generatedFiles().find(file => file.path === "src/pages/api/funnel-event.ts")?.contents ?? "";
    ...
    expect(endpoint).toContain(":append?valueInputOption=RAW&insertDataOption=INSERT_ROWS");
  ...
  it("authenticates stage callbacks and emits the canonical deduplicated events", () => {
    const endpoint = generatedFiles().find(file => file.path === "src/pages/api/lead-stage.ts")?.contents ?? "";
    expect(endpoint).toContain('request.headers.get("authorization")?.replace(/^Bearer\\s+/i, "")');
    expect(endpoint).toContain("STAGE_WEBHOOK_SECRET");
    expect(endpoint).toContain('{ qualified: "QualifiedLead", appointment: "Schedule", show: "Showed", sale: "Purchase" }');
    expect(endpoint).toContain('const externalId = leadUuid + ":" + stage');
    expect(endpoint).toContain("conversion?.event_id || eventId");
    expect(endpoint).toContain('stage === "sale" && (!Number.isFinite(value) || value <= 0)');
    expect(endpoint).toContain("first_url: lead.first_url");
    expect(endpoint).toContain("original_query_string: lead.original_query_string");
  ...
  it("keeps secret values out of browser files and browser-safe profile DTOs", () => {
    const browserSource = generatedFiles()
      .filter(file => file.path.startsWith("public/") || file.path.endsWith(".astro"))
      .map(file => file.contents)
      .join("\n");
    const fixtureSecrets = buildReadyPaidFunnelSecrets() as Record<string, string | undefined>;
    const secretValues = SIMPLE_FORM_OFFLINE_CONVERSION_CONTRACT.requiredRuntimeSecrets
