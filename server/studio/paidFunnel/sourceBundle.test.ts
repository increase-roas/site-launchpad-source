...
describe("generic paid-funnel source bundle", () => {
  it("wires one saved customer profile into runtime bindings without compiling secrets", () => {
    ...
    expect(result.runtimeVars).toEqual(expect.objectContaining({
      GHL_LOCATION_ID: "location-123",
      GOOGLE_SHEETS_ID: "sheet-123",
      META_PIXEL_ID: "123456789012345",
      META_GRAPH_API_VERSION: "v26.0",
    }));
    expect(result.bindings.bindingNames).toEqual(expect.arrayContaining([
      "GHL_API_KEY",
    ...
    ] as const) expect(source).not.toContain(result.bindings.secrets[name]!);
  });
