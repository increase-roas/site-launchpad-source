...
export const simpleFormOperatorProductSchema = z.object({
  id: z
    .string()
    .trim()
    .regex(productId, "Use lowercase letters, numbers, and hyphens."),
...
export type SimpleFormOperatorConfig = z.infer<
  typeof simpleFormOperatorConfigSchema
>;
  ...
  .superRefine((config, context) => {
    ...
    if (config.googleEnhancedConversions && !config.ga4MeasurementId) {
      context.addIssue({
        code: "custom",
        message:
          "ga4MeasurementId is required when Google enhanced conversions are enabled.",
...
export type SimpleFormStoredRecord = z.infer<
  typeof simpleFormStoredRecordSchema
>;
  ...
  [
    {
      ...
      description:
        "In stock now — hydrotherapy jets, LED lighting, energy-smart cover.",
      imageUrl:
    ...
    },
    {
      ...
      description:
        "Active floor model — swim current, seating zone, low-maintenance shell.",
      imageUrl:
    ...
    },
    {
      ...
      description:
        "Showroom favorite — lounge seating, waterfall, and smart controls.",
      imageUrl:
...
export function defaultSimpleFormImageSources(): SimpleFormStoredRecord["imageSources"] {
  return {
    logo: { mode: "template" },
    products: SIMPLE_FORM_TEMPLATE_PRODUCTS.map(() => ({
...
export function buildSimpleFormOperatorDefaults(input: {
  ...
  return {
    ...
    funnel: {
      ...
      qualifyingLine:
        "For homeowners inside the local delivery and service area.",
    },
    offer: {
...
export function buildSimpleFormReadiness(
  ...
  if (!e164.test(config.client.phone.trim()))
    clientMissing.push("Client phone (E.164)");
  if (!config.client.logoUrl.trim()) clientMissing.push("Logo");

  const offerMissing: string[] = [];
  if (config.offer.headline.trim().length < 12)
    offerMissing.push("Offer headline");
  if (config.offer.subheadline.trim().length < 24)
    offerMissing.push("Offer subheadline");

  const zips = Array.from(
    new Set(config.serviceAreaZipCodes.map(zip => zip.trim()).filter(Boolean))
  );
  const serviceMissing: string[] = [];
  if (zips.length < 1) serviceMissing.push("At least one service-area ZIP");
  if (zips.some(zip => !zip5.test(zip)))
    serviceMissing.push("Every ZIP must be 5 digits");
  if (
    !config.geoH1Template.includes("{city}") ||
    !config.geoH1Template.includes("{state}")
  ) {
    serviceMissing.push("ZIP headline must include {city} and {state}");
  }

  const metaMissing: string[] = [];
  if (!pixelId.test(integration.META_PIXEL_ID?.trim() ?? ""))
    metaMissing.push("Meta Pixel ID");
  if (!secrets.META_CAPI_ACCESS_TOKEN)
    metaMissing.push("Meta CAPI Access Token");
  if (
    config.googleEnhancedConversions &&
    !/^G-[A-Z0-9]{6,20}$/i.test(config.ga4MeasurementId ?? "")
  ) {
  ...
  if (!integration.GOOGLE_SHEETS_ID?.trim())
    googleSheetsMissing.push("Google Sheet ID");
  if (!secrets.GOOGLE_SERVICE_ACCOUNT_EMAIL) {
    googleSheetsMissing.push("Google service-account email");
  }
  if (!secrets.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY) {
  ...
  if (!secrets.STAGE_WEBHOOK_SECRET)
    secretMissing.push("Lifecycle Callback Secret");

  const canonicalResult =
    simpleFormCanonicalShapeAConfigSchema.safeParse(config);
  ...
  const sections: SimpleFormReadinessSection[] = [
    {
      key: "client",
      label: "Client",
      ready: clientMissing.length === 0,
      missing: clientMissing,
    },
    {
      key: "offer",
      label: "Offer",
      ready: offerMissing.length === 0,
      missing: offerMissing,
    },
    {
      key: "serviceArea",
      label: "Service Area",
      ready: serviceMissing.length === 0,
      missing: serviceMissing,
    },
    {
      key: "meta",
      label: "Meta",
      ready: metaMissing.length === 0,
      missing: metaMissing,
    },
    {
      key: "ghl",
      label: "GHL",
      ready: ghlMissing.length === 0,
      missing: ghlMissing,
    },
    {
      key: "googleSheets",
      ...
      missing: googleSheetsMissing,
    },
    {
      key: "inventory",
      label: "Inventory",
      ready: inventoryMissing.length === 0,
...
export function resolveSimpleFormImages(
  ...
  const logoUrl =
    record.imageSources.logo.mode === "client-media"
      ? (bySlot.get(record.imageSources.logo.slot) ??
  ...
  const products = record.config.inventory.products.map((product, index) => {
    ...
    const templateUrl =
      SIMPLE_FORM_TEMPLATE_PRODUCTS[index]?.imageUrl ?? product.imageUrl;
    const imageUrl =
      source?.mode === "client-media"
        ? (bySlot.get(source.slot) ?? templateUrl)
        : templateUrl;
...
export function simpleFormFunnelSlug(
  shortName: string,
