...
describe("generic multi-step paid funnel UX", () => {
  it("ships every paid-ad section preset and a complete opt-in/survey/form funnel", () => {
    ...
    expect(GENERIC_PAID_FUNNEL_PACKAGE.offlineConversionContract.purchase.requiresExplicitPositiveValue).toBe(true);
  });

  it("renders a real nested canvas per step, not a flat block list", () => {
    const graph = createGenericPaidFunnelFixture();
    const landing = renderFunnelCanvas(graph, { stepKey: "landing", breakpoint: "desktop" });
    const form = renderFunnelCanvas(graph, { stepKey: "form", breakpoint: "desktop" });
    ...
    expect(landing?.children[0]?.children[0]?.children[0]?.children[0]?.kind).toBe("element");
    const landingLabels = flattenCanvas(landing).filter(box => box.kind === "element").map(box => box.label);
    const formLabels = flattenCanvas(form).filter(box => box.kind === "element").map(box => box.label);
  ...
  it("exposes full inspector controls and applies global styles plus per-node responsive overrides", () => {
    ...
    expect(inspectorModel(graph, section.id, "desktop")?.controls).toEqual(expect.arrayContaining([
    ...
    expect(inspectorModel(graph, row.id, "desktop")?.controls).toEqual(expect.arrayContaining(["gap", "valign", "wrap", "background", "padding", "columns"]));
    expect(inspectorModel(graph, column.id, "tablet")?.controls).toEqual(expect.arrayContaining(["width", "alignment", "padding", "background", "border", "visibility", "elementOrder"]));
    expect(inspectorModel(graph, element.id, "mobile")?.controls).toEqual(expect.arrayContaining([
      "typography", "spacing", "color", "border", "alignment", "link", "action", "responsive", "visibility", "duplicate", "delete",
    ]));
    const restyled = applyGlobalStyles(graph, { fonts: { heading: "Oswald", body: "Inter" }, button: { ...graph.globalStyles.button, background: "#f97316" } });
  ...
  it("autosaves, detects revision conflicts, and undoes structural plus property edits", () => {
    ...
    const heading = state.document.graph.pages.landing.sections[0]!.rows[0]!.columns[0]!.elements.find(el => el.type === "heading")!;
    state = applyGraph(state, setInlineText(state.document.graph, heading.id, "Edited headline"));
    ...
    const undone = findNode(state.document.graph, heading.id)?.node as { props: { text: string } };
    expect(undone.props.text).not.toBe("Edited headline");
    state = studioHotkey(state, "z", { meta: true, shift: true });
    const redone = findNode(state.document.graph, heading.id)?.node as { props: { text: string } };
  ...
  it("migrates only versioned paid-funnel graphs and rejects website block lists", () => {
    ...
    expect(() => migratePaidFunnelGraph({ schemaVersion: 0, blocks: [{ type: "hero" }] })).toThrow(/block lists/);
    const reused = saveReusableSection(graph, graph.pages.landing.sections[0]!.id, "Shared hero");
