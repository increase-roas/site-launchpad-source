import type { Background, BoxSpacing, FunnelElement, GlobalFunnelStyles, PaidFunnelBreakpoint, PaidFunnelGraph, ResponsiveValue } from "./graph";
...
function pickResponsive<T>(value: ResponsiveValue<T> | undefined, breakpoint: PaidFunnelBreakpoint): T | undefined {
...
function backgroundCss(background: Background | undefined, fallback: string): string {
  if (!background || background.kind === "none") return fallback;
  if (background.kind === "color") return background.color;
  if (background.kind === "gradient") return `linear-gradient(${background.angle}deg, ${background.from}, ${background.to})`;
  if (background.kind === "image") return `center / ${background.size} url(${background.url})`;
  return fallback;
}

export function elementText(element: FunnelElement): string {
  if (element.type === "button" || element.type === "phoneCta") return String(element.props.label ?? "");
...
export function renderFunnelCanvas(
  ...
  input: { stepKey: string; breakpoint: PaidFunnelBreakpoint; selectedId?: string | null },
): CanvasBox | null {
  ...
  return {
    ...
    style: { background: styles.colors.background, color: styles.colors.text, fontFamily: styles.fonts.body },
    children: page.sections.map(section => {
      const visible = section.visibility[input.breakpoint];
      const padding = pickResponsive(section.padding, input.breakpoint) ?? (input.breakpoint === "mobile" ? styles.mobile.sectionPadding : undefined);
      return {
        id: section.id,
        kind: "section",
        ...
        style: {
          ...
          maxWidth: section.layout === "boxed" ? section.maxWidth : styles.containers.fullMaxWidth,
          minHeight: section.minHeight,
          padding: spacingCss(padding) ?? "0",
          margin: spacingCss(pickResponsive(section.margin, input.breakpoint)) ?? "0",
        ...
        children: section.rows.map(row => ({
          ...
          style: {
            ...
            alignItems: row.valign === "center" ? "center" : row.valign === "bottom" ? "flex-end" : "flex-start",
            gap: input.breakpoint === "mobile" ? styles.mobile.rowGap : row.gap,
            padding: spacingCss(pickResponsive(row.padding, input.breakpoint)) ?? "0",
          ...
          children: row.columns.map(column => {
            ...
            return {
              ...
              style: {
                ...
                width: `${column.widths[input.breakpoint]}%`,
                padding: spacingCss(pickResponsive(column.padding, input.breakpoint)) ?? "0",
                background: backgroundCss(column.background, "transparent"),
                textAlign: column.alignment,
                border: `${column.borderWidth}px solid ${column.borderColor}`,
              },
              children: column.elements.map(element => renderElement(element, input.breakpoint, input.selectedId, styles)),
            };
          }),
        })),
      };
...
function renderElement(
  ...
  return {
    ...
    style: {
      ...
      borderRadius: element.styles.borderRadius ?? (isButton ? styles.button.radius : 0),
    },
    children: [],
  };
}
...
export function dropIndexFromPointer(childCount: number, pointerY: number, top: number, height: number): number {
