...
function cssSource(graph: PaidFunnelGraph): string {
  ...
  return `:root{--background:${colors.background};--surface:${colors.surface};--heading-text:${colors.heading};--text:${colors.text};--muted:${colors.muted};--primary:${colors.primary};--primary-text:${colors.primaryText};--border:${colors.border};--heading:${fonts.heading};--body:${fonts.body}}*{box-sizing:border-box}body{margin:0;background:var(--background);color:var(--text);font-family:var(--body),system-ui,sans-serif}main{min-height:100vh}.section{padding:64px 24px}.row{display:flex;gap:24px;max-width:${containers.boxedMaxWidth}px;margin:0 auto}.column{display:flex;flex-direction:column;gap:18px;width:var(--desktop-width)}h1,h2,h3{color:var(--heading-text);font-family:var(--heading),system-ui,sans-serif;margin:0;line-height:1.05}h1{font-size:clamp(2.25rem,6vw,4.5rem)}p{color:var(--text);font-size:1.1rem;line-height:1.65}.funnel-button,.choice{display:inline-flex;justify-content:center;border:0;border-radius:${button.radius}px;background:${button.background};color:${button.color};padding:${button.paddingY}px ${button.paddingX}px;font-weight:${button.fontWeight};text-decoration:none;cursor:pointer}.funnel-form,.survey-question{display:grid;gap:12px;width:min(100%,520px);margin:0 auto}.funnel-form label{display:grid;gap:6px}.funnel-form input{min-height:48px;border:1px solid var(--border);border-radius:8px;padding:0 14px;font:inherit}.funnel-form .consent{grid-template-columns:auto 1fr;align-items:start;color:var(--muted)}.funnel-form .consent input{min-height:0;margin-top:4px}blockquote footer,input::placeholder{color:var(--muted)}.funnel-error{color:#b91c1c;font-size:.95rem;font-weight:700}.choice{width:100%;background:var(--surface);color:var(--text);border:1px solid var(--border);text-align:left}.choice[data-selected=true]{border-color:var(--primary);box-shadow:0 0 0 2px color-mix(in srgb,var(--primary) 20%,transparent)}img{display:block;max-width:100%;height:auto;border-radius:12px}@media(max-width:720px){.section{padding:40px 16px}.row{flex-direction:column}.column{width:100%}}`;
}
