...
<button class="funnel-button" type="submit">${html(props.submitLabel ?? "Continue")}</button>
  ...
  if (element.type === "multipleChoice") {
    ...
    const gap = Math.max(0, Math.min(80, Number(props.gap) || 12));
    const buttonBackground = cssText(props.buttonBackground, "var(--surface)");
    const buttonColor = cssText(props.buttonColor, "var(--text)");
    const buttonRadius = Math.max(
      0,
      Math.min(100, Number(props.buttonRadius) || 0)
    );
    return `<div class="${classes("funnel-element", elementClass, "survey-question")}" style="--choice-columns:${columns};--choice-gap:${gap}px;--choice-background:${html(buttonBackground)};--choice-color:${html(buttonColor)};--choice-radius:${buttonRadius}px" data-block-id="${html(element.id)}" data-survey-question data-field="${html(field)}" data-destination="${html(destination(step.nextStep, graph))}" data-auto-advance="${props.autoAdvance === false ? "false" : "true"}">
<p class="survey-question-label">${html(props.question ?? "Choose an option")}</p>
            ...
            const visibleDisplay =
              ...
              element.type === "multipleChoice"
                ...
                : isButton
                  ? "inline-flex"
                  : "block";
            const elementWidth = responsiveValue(
              element.styles.width,
              breakpoint
            );
            const elementMaxWidth = responsiveValue(
                ...
                [
                  ...
                  elementMaxWidth === undefined
                    ? element.type === "image" ||
                      ...
                      ? "100%"
                      : "none"
                    : `${finite(elementMaxWidth, 1120, 1, 4000)}px`,
                ],
                [
                  "color",
                  cssText(
